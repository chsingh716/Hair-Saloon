...
import static uk.ac.tees.S6145076.registerUser.ADDRESS_USER;
import static uk.ac.tees.S6145076.registerUser.MAP_CODE_REQUEST;
                ...
                public void onComplete(@NonNull Task<Location> task) {
                    if (task.isSuccessful()) {
                        ...
                        if (getCurrentLocation != null) {
                           ...
                           _map.addMarker(new MarkerOptions().position(new LatLng(getCurrentLocation.getLatitude(), getCurrentLocation.getLongitude())).title("My Location"));

//                            CameraUpdate location = CameraUpdateFactory.newLatLngZoom(
//                                    _dummy, DEFAULT_ZOOM);

                            CameraUpdate location = CameraUpdateFactory.newLatLngZoom(
                                    new LatLng(getCurrentLocation.getLatitude(), getCurrentLocation.getLongitude()), DEFAULT_ZOOM);
...
//        LatLng _dummy = new LatLng(54.5742982466006, -1.2349123090100282);
        ...
        try {
           // addresses = geocoder.getFromLocation(_dummy.latitude, _dummy.longitude, 1);
            addresses = geoc.getFromLocation(getCurrentLocation.getLatitude(), getCurrentLocation.getLongitude(), 1); // Here 1 represent max location result is returned, range varied from 1 to 5
